<div class="bg-white rounded-xl shadow overflow-hidden">
    <div class="p-6 border-b border-gray-200 flex items-center justify-between">
        <h2 class="text-lg font-semibold text-gray-800">Transit Media Report</h2>
        ...
        <table class="min-w-full text-sm">
            <thead class="bg-gray-50 text-gray-600 uppercase tracking-wider text-xs">
                <tr>
                    ...
                    <th class="px-6 py-3 text-left">Total Number Commisioned In Ibeju-Lekki</th>
            ...
            <tbody class="divide-y divide-gray-100">
                @foreach ($transitMediaReports as $report)
                    <tr class="hover:bg-gray-50 transition">
                        <td class="px-6 py-4 font-medium text-gray-800">{{ $report['Group'] }}</td>
                        <td class="px-6 py-4 text-indigo-600 font-bold">{{ $report['Mode'] }}</td>
                        ...
                        <td class="px-6 py-4 text-green-700 font-bold">{{ $report['Total Number Commisioned In Ibeju-Lekki'] }}</td>
                        <td class="px-6 py-4 text-green-700 font-bold">{{ $report['Total Number In Epe LGA'] }}</td>
                        <td class="px-6 py-4 text-yellow-600 font-bold">{{ $report['Total Number Commissioned In Epe'] }}</td>
                    </tr>
                @endforeach
            </tbody>
        </table>
